...
Filling in the missing values has raised the mean number of steps taken per day from ```r round(mean(filter(stepsPerDay2, ver == "Original data")$steps), digits = 2)``` to ```r format(round(mean(filter(stepsPerDay2, ver == "NAs removed")$steps), digits = 2), scientific=FALSE)```. The median and mean in the new data are equal.
